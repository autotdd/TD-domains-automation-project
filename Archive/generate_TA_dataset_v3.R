...
create_ta_domain <- function(study_id, trial_design, arms_data, treatments, output_dir = getwd()) {
  ...
  generate_elements <- function(epochs, arm_index, trial_design, treatments) {
    num_treatments <- length(treatments)
    start_index <- (arm_index %% num_treatments) + 1
    treatment_order <- c(treatments[start_index:num_treatments], treatments[1:(start_index-1)])
    print(paste("Arm index:", arm_index))
    print(paste("Treatment order:", paste(treatment_order, collapse = ", ")))
    treatment_counter <- 0
    elements <- sapply(seq_along(epochs), function(i) {
      if (grepl("TREATMENT", epochs[i], ignore.case = TRUE)) {
        ...
        element <- paste0("TREATMENT ", treatment_order[treatment_index])
        treatment_counter <<- treatment_counter + 1
        print(paste("Epoch:", epochs[i], "-> Element:", element))
        return(element)
      } else {
        return(epochs[i])
      }
  ...
  for (i in seq_along(arms_data)) {
    ...
    element_descriptions <- generate_elements(epochs, i, trial_design, treatments)
...
treatments <- c("A", "B", "C") # Define the treatments dynamically
